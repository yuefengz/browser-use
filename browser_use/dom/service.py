...
class DomService:
    ...
    def __init__(self, page: "Page", logger: logging.Logger | None = None):
        ...
        self.js_code = (
            resources.files("browser_use.dom.dom_tree").joinpath("index.js").read_text()
        )
    ...
    @observe_debug(ignore_input=True, ignore_output=True, name="get_hoverable_elements")
    @time_execution_async("--get_hoverable_elements")
    async def get_hoverable_elements(
        self,
        highlight_elements: bool = False,
        focus_element: int = -1,
        viewport_expansion: int = 0,
    ) -> DOMState:
        """Get all hoverable elements that show/hide content or change appearance on hover.

        Args:
                highlight_elements: Whether to visually highlight elements (default False)
                focus_element: Element index to focus on (-1 for no focus)
                viewport_expansion: Pixels to expand viewport detection area

        Returns:
                DOMState containing element tree and selector map for hoverable elements
        """
        element_tree, selector_map = await self._build_dom_tree(
            highlight_elements, focus_element, viewport_expansion
        )
    ...
    async def _build_dom_tree(
        ...
        try:
            self.logger.debug(
                f"🔧 Starting JavaScript DOM analysis for {self.page.url[:50]}..."
            )
        ...
        if debug_mode and "perfMetrics" in eval_page:
            ...
            url_short = (
                self.page.url[:50] + "..." if len(self.page.url) > 50 else self.page.url
            )
            self.logger.debug(
    ...
    def _parse_node(
        ...
        element_node = DOMElementNode(
            ...
            is_hoverable=node_data.get("isHoverable", False),
