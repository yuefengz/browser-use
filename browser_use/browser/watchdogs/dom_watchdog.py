...
class DOMWatchdog(BaseWatchdog):
	...
	async def _build_dom_tree(self, previous_state: SerializedDOMState | None = None) -> SerializedDOMState:
		...
		try:
			...
			elif self.selector_map and self._dom_service:
				self.logger.debug(f'🔍 DOMWatchdog._build_dom_tree: Highlighting disabled (highlight_elements=False), skipping injection for {len(self.selector_map)} elements')
